...
Next, I knew that I needed to include a loop, so that we could calculate the NPV over a multiple years. To do so, I decided to employ the purrr function, map2_dbl. Finally, I used sum() to determine the total NPV, regardless of the number of years of data that are calculated. 
...
3. While DESCRIPTION and NAMESPACE are both created with the package, the function R document was created using use_r("NPV"), and the README and vignette files were created and built using use_readme_rmd(), build_readme(), use_vignette("Calculating_NPVs"), and build_vignettes(). Both the README file and the vignette were manually configured and written by me. 
...
5. To run tests, I used use_testthat() followed by use_test("NPV") to create a test files for my function.Then, I manually created the tests. 
